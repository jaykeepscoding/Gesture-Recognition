...
def gen_model(input_sz, num_classes):
    ...
    L2 = tf.keras.layers.Dense(600, activation='relu')(L1)
    L3 = tf.keras.layers.Dense(300, activation='relu')(L2)
    L4 = tf.keras.layers.Dense(200, activation='relu')(L3)
    L5 = tf.keras.layers.Dense(100, activation='relu')(L4)
    L6 = tf.keras.layers.Dense(50, activation='relu')(L5)
    L7 = tf.keras.layers.Dense(32, activation='relu')(L6)
    L8 = tf.keras.layers.Dense(25, activation='relu')(L7)
    L9 = tf.keras.layers.Dense(16, activation='relu')(L8)

    L10 = tf.keras.layers.Dense(num_classes, activation='softmax')(L9)
    model = tf.keras.models.Model(inputs=inputs, outputs=L10)
